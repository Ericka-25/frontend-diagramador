...
export const InputsAtributes = ({ index,atributo, tipos, deleteAtributo, primaryKey ,changeAtributo,changeTipo,changePK}) => {
    ...
    return (
        <div>
            <input type="text" name="titulo" onChange={handleatributoChange} value={atributos} className='px-2 bg-[##FFFA] rounded border border-gray-600' />
            <SelectOption
                options={options}
            ...
            <button onClick={handleDelete} className='mx-1 bg-red-600 rounded px-1 text-white'>Delete</button>
        </div>
    )
}
